...
- **Landing Page（营销首页）：** `http://localhost:8001/` — 支持 **中文 / English**（右上角切换，默认中文），语言偏好存 `localStorage`。页面结构：**宏观洞察**（Hero + 雷达动画与悬浮卡）→ **深度匹配**（三列能力）→ **自动化执行**（步骤说明 + SQLite 示意）→ 底部 CTA；顶栏锚点与滚动同步高亮。
- **工作台（原操作台）：** [http://localhost:8001/workbench](http://localhost:8001/workbench)
...
```
# DeepSeek（分析、赛道、匹配、过滤、差距分析等共用）
DEEPSEEK_API_KEY=your_key
DEEPSEEK_BASE_URL=https://api.deepseek.com
DEEPSEEK_MODEL_V4_FLASH=deepseek-v4-flash
DEEPSEEK_MODEL_V4_PRO=deepseek-v4-pro
# 可选：仍可用旧名 DEEPSEEK_MODEL_CHAT / DEEPSEEK_MODEL_REASONER 代替上面两行
```

可选 **Supermind**：`SUPER_MIND_API_KEY`、`SUPER_MIND_BASE_URL`、`SUPER_MIND_MODEL`（与工作台并存时，模型列表会多一项）。
...
├── deepseek_env.py         # DeepSeek：API model 名 + 环境变量（兼容旧名）
